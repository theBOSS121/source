...
import com.bombit.BinaryWriter;
...
public class Client {
	...
	private void process(DatagramPacket packet){
		...
		if (!connected){
			...
			int x = ByteBuffer.wrap(data, 4, 4).getInt();
			int y = ByteBuffer.wrap(data, 8, 4).getInt();
			Game.player.x = x;
			Game.player.y = y;
			connected = true;
		}else if(new String(data, 0, 3).equals("NEW")){
			if(clients.size() + 1 == ByteBuffer.wrap(data, 3, 4).getInt()){
					int x = ByteBuffer.wrap(data, ((ByteBuffer.wrap(data, 3, 4).getInt() - 1) * 16) + 7, 4).getInt();
					int y = ByteBuffer.wrap(data, ((ByteBuffer.wrap(data, 3, 4).getInt() - 1) * 16 + 4) + 7, 4).getInt();
					int spriteNum = ByteBuffer.wrap(data, ((ByteBuffer.wrap(data, 3, 4).getInt() - 1) * 16 + 8) + 7, 4).getInt();
				...
				for(int i = 0; i < ByteBuffer.wrap(data, 3, 4).getInt(); i++){
					int x = ByteBuffer.wrap(data,7 + (i * 16), 4).getInt();
					int y = ByteBuffer.wrap(data,4 + 7 + (i * 16), 4).getInt();
					int spriteNum = ByteBuffer.wrap(data,8 + 7 + (i * 16), 4).getInt();
					Sprite sprite = getSprite(spriteNum);
					clients.add(new ServerClient(x, y, sprite));
				}
				...
				for(int i = 0; i < ByteBuffer.wrap(data, 3, 4).getInt(); i++){
					clients.get(i).x = ByteBuffer.wrap(data, 7 + (i * 16), 4).getInt();
					clients.get(i).y = ByteBuffer.wrap(data, 4 + 7 + (i * 16), 4).getInt();
					clients.get(i).sprite = getSprite(ByteBuffer.wrap(data, 8 + 7 + (i * 16), 4).getInt());
					clients.get(i).plantBomb((ByteBuffer.wrap(data, 12 + 7 + (i * 16), 4).getInt()));
				}
			}
		}
	}
